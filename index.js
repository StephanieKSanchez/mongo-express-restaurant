...
let menudb, customerdb // these are so that that mongo does not give warnings
...
mongoClient.connect(url, options, (err, mongoClient) => {
    ...
    app.listen(3100, () => console.log('App is listening on port 3100'))

    const db = mongoClient.db('restaurant')
    customersdb = db.collection('customers') // assigning a 'customer' collection to the customerdb
